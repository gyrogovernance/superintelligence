...
Computes physical information capacity, CSM for k-byte hQVM Kernel states,
...
def csm(k_bytes: int) -> float:
    """Common Source Moment: microcells per hQVM Kernel state for k-byte keys."""
...
def bytes_backing_per_router_state(k_bytes: int, s_bits: float) -> float:
    """Bytes needed per hQVM Kernel state when backing with s bits per microcell."""
...
def run_full_report(k_range: range = range(1, 17), s_values: dict | None = None) -> None:
    ...
    # CSM table
    print("CSM (microcells per hQVM Kernel state):")
    for k in k_range:
        omega = 2 ** (8 * k)
    ...
    # hQVM Kernel backing
    for k in (2, 4):
        print(f"hQVM Kernel k={k} bytes (CSM={csm(k):.6e} cells/state):")
        for name, s in s_values.items():
