...
The report proceeds from physical principles to their discrete realisation in the kernel, with each claim verified by the physics test suite (tests use the kernel API only; no atlas).
...
This closed form is verified by the physics tests via the kernel API (exhaustive or sampled checks as appropriate). The kernel is an affine dynamical system on a discrete phase space.
...
Physics verification tests call the kernel API directly. To run all physics tests:
```
