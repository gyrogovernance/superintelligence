...
The kernel operates on a finite reachable shared-moment space Ω with exactly 4,096 states from rest.
...
Let C₆₄ be the kernel's 64-element 12-bit mask code. Then:

- A_set = { archetype_A XOR c : c in C₆₄ }
- B_set = { archetype_B XOR c : c in C₆₄ }
- Ω = A_set × B_set
- \|Ω\| = 64 × 64 = 4,096

The kernel is **byte-complete**: all bytes 0..255 are valid actions.

#### 2.2 Holographic boundary (Horizons)

The kernel defines two distinguished 64-state boundary sets within Ω:

- Complement horizon (S-sector): states where `A = B XOR 0xFFF`
- Equality horizon (UNA degeneracy): states where `A = B`

Both horizons satisfy the holographic relation \|H\|² = \|Ω\| = 64² = 4,096 and are related by intrinsic gates. A 4-to-1 holographic dictionary maps pairs (horizon state, byte) to Ω: every Ω state has exactly four horizon-plus-byte preimages.
...
#### 2.6 Runtime and observables

The kernel's runtime stepping and observables are provided directly by the spinorial transition law and precomputed per-byte tables (masks, families, chirality words). Kernel stepping at runtime is an O(1) application of the algebraic transition rule on the 24-bit carrier; observables such as horizon distance, vertex charge, chirality word, and mask weight are computed from the current state and the per-byte tables.
...
Both the kernel and transformer architectures are built on the same binary computational medium. All structurally significant dimensions in both systems sit on powers of 2 (and occasionally 2^n * 3^m): the kernel's reachable shared-moment space (4,096 = 2^12), horizons (64 = 2^6), mask code (12 bits), vertex classes (4 = 2^2), and phase (4 = 2^2) share the same arithmetic foundation as the transformer's hidden size (4,096 = 2^12), layer count (32 = 2^5), head count (32 = 2^5), and position capacity (65,536 = 2^16). This is not coincidence or numerology. It is a consequence of both systems being implemented in binary digital computation, which is itself constrained by physics.
