...
This is a data-only ``GenomicCompile`` / ``compile_interval`` adapter. It
...
into the 9-layer compile record. Catalog files live in this package under
``data/dataset_genomics/`` (filled by ``ingest_genomics.py`` from public URLs).

The certified layers (names kept identical for application compatibility):
  byte_fold_w, fold_poles, family_sheet, omega_signature, depth4_parity,
  chi_shells, chi_shell_census, chi_bit_occupation, qubec_order, ab_horizon,
  boundary_keys.
...
# Fold pole and family are intron observables; index through intron_from_byte.
_INTRONS_BY_BYTE = np.asarray(
    [intron_from_byte(b, CHIRALITY_D) for b in range(256)],
    dtype=np.int64,
)
_FOLD_POLE_BITS = np.asarray(
    [((int(i) >> 2) & 0b11) for i in _INTRONS_BY_BYTE],
    dtype=np.int64,
)
_INTRON_FAMILY_IDX = np.asarray(
    [intron_family_d(int(i), CHIRALITY_D) for i in _INTRONS_BY_BYTE],
    dtype=np.int64,
)
...
def reverse_complement_acgt(seq: str) -> str:
    return seq.upper().replace("U", "T").translate(_RC_TABLE)[::-1]


def is_pair_inversion_encoding(enc: NucleotideEncoding) -> bool:
    """True when Watson-Crick complement is antipodal transport phi ^ 3."""
    comp = {"A": "T", "T": "A", "G": "C", "C": "G"}
    return all(enc.encode_base(comp[b]) == (enc.encode_base(b) ^ 3) for b in BASES)


def pair_inversion_encoding() -> NucleotideEncoding:
    """First certified pair-inversion nucleotide chart (duplex antipode)."""
    for enc in all_nucleotide_encodings():
        if is_pair_inversion_encoding(enc):
            return enc
    raise RuntimeError("no pair-inversion encoding among the 24 charts")


def kinematic_rc_byte(byte: int, enc: NucleotideEncoding) -> int:
    """Four-mer reverse complement compiled back to a byte on this chart.

    Distinct from shadow_partner_byte (single-step carrier degeneracy) and
    from payload reverse complement. Analysis_hQVM_CGM_Genomics.md Sec. 2-3.
    """
    mer = unpack_4mer_byte(int(byte) & 0xFF, enc)
    return pack_4mer_byte(reverse_complement_acgt(mer), enc)


@lru_cache(maxsize=24)
def kinematic_rc_byte_table(phi: Tuple[int, int, int, int]) -> Tuple[int, ...]:
    enc = next(e for e in all_nucleotide_encodings() if e.phi == phi)
    return tuple(kinematic_rc_byte(b, enc) for b in range(256))
...
def compile_interval(
    ...
    *,
    ...
    """Compile one sequence window into the certified GenomicCompile record.
    ...
    ``certificates=False`` skips the ``ab_horizon`` layer, which is a path
    certificate that no design objective reads. That layer costs about a third
    of the compile, so an inner design loop can skip it and still be scored on
    every layer that steers a target. The layer reports NaN in that mode, which
    is visible rather than silent, and any molecule that is actually returned
    is recompiled with ``certificates=True``.
    """
    s = clean_acgt(seq)
    ...
    for u, v in zip(packed_codons, packed_codons[1:]):
        ...
        chi_shells.append(chirality_shell(chi))
    n_chi = len(chi_shells)
    mean_shell = (sum(chi_shells) / n_chi) if n_chi else float("nan")
    if certificates:
        mean_ab, mean_hor, n_ab_pairs = _interval_ab_horizon(codon_list, enc)
    else:
        mean_ab = mean_hor = float("nan")
        n_ab_pairs = 0
    if n_chi:
        chi_shell_arr = np.asarray(chi_shells, dtype=np.int64)
        shell_census = np.bincount(chi_shell_arr, minlength=7).astype(np.float64) / n_chi
        chi_reg_arr = np.asarray(chi_values, dtype=np.int64)
        # Per-mode damping: rate at which each payload bit of the chirality
        # register is carried. Equals 0.5 under a uniform (thermal) register.
        bit_occupation = np.asarray(
            [float(((chi_reg_arr >> k) & 1).mean()) for k in range(CHIRALITY_D)],
            dtype=np.float64,
        )
    else:
        shell_census = np.full(7, np.nan)
        bit_occupation = np.full(CHIRALITY_D, np.nan)
    ...
    layers = (
        ...
        LayerReport("chi_shells", (
            ...
            ("low_shell_fraction", float(shell_census[:2].sum())),
        )),
        LayerReport("chi_shell_census", tuple(
            (f"shell_{k}_frac", float(shell_census[k])) for k in range(7)
        )),
        LayerReport("chi_bit_occupation", tuple(
            (f"bit_{k}_frac", float(bit_occupation[k])) for k in range(CHIRALITY_D)
        )),
...
def compile_climate_summary(gc_obj: GenomicCompile) -> Dict[str, float]:
    """Five-column climate summary read from the certified compile.
    ...
    return {
        ...
        "mean_fold_disagreement": _sum("byte_fold_w", "mean_fold_disagreement"),
