...
def _build_shared_library(src: Path, *, opencl: bool = False) -> Path:
    ...
    stem = src.stem
    out_name = f"{stem}_{_hash_source(src)}{_shared_lib_suffix()}"
    ...
    if kind == "msvc":
        cmd = [compiler, "/nologo", "/O2", "/LD", "/TC", str(src), f"/Fe:{out_path}"]
        if opencl:
        ...
        flags = ["-O3", "-std=c11", "-shared", *extra_flags]
...
def _setup_cpu_argtypes(lib: ctypes.CDLL) -> None:
    ...
    if hasattr(lib, "gyrograph_ingest_word4_batch"):
        lib.gyrograph_ingest_word4_batch.argtypes = [
        ...
        lib.gyrograph_ingest_word4_batch.restype = None

    if hasattr(lib, "gyrograph_trace_word4_batch"):
        lib.gyrograph_trace_word4_batch.argtypes = [
        ...
        lib.gyrograph_trace_word4_batch.restype = None

    if hasattr(lib, "gyrograph_apply_trace_word4_batch"):
        lib.gyrograph_apply_trace_word4_batch.argtypes = [
            ctypes.c_void_p,  # omega12_io
            ctypes.c_void_p,  # step_io
            ctypes.c_void_p,  # last_byte_io
            ctypes.c_void_p,  # has_closed_word_io
        ...
        lib.gyrograph_apply_trace_word4_batch.restype = None
...
def _check_ingest_shapes(
    ...
    expected_1d = {
        "step": step,
        "last_byte": last_byte,
        "has_closed_word": has_closed_word,
        "chi_ring_pos": chi_ring_pos,
        "chi_valid_len": chi_valid_len,
        "omega_sig": omega_sig,
        "parity_O12": parity_O12,
        "parity_E12": parity_E12,
        "parity_bit": parity_bit,
    }
    for name, arr in expected_1d.items():
        if arr.shape != (n,):
            raise ValueError(f"{name} must have shape {(n,)}, got {arr.shape}")

    expected_2d = {
        "word4_store": (word4_store, (n, 4)),
        "chi_ring64": (chi_ring64, (n, 64)),
        "chi_hist64": (chi_hist64, (n, 64)),
        "shell_hist7": (shell_hist7, (n, 7)),
        "words4": (words4, (n, 4)),
    }
    for name, (arr, shape) in expected_2d.items():
        if arr.shape != shape:
...
def _py_apply_trace_word4_batch(
    omega12: np.ndarray,
    step: np.ndarray,
    last_byte: np.ndarray,
    has_closed_word: np.ndarray,
    ...
    for i in range(n):
        ...
        for k in range(4):
            ...
            ring_pos_new, valid_len_new = _py_push_chi_row(
                chi_ring64[i],
                int(chi_ring_pos[i]),
                int(chi_valid_len[i]),
                chi_hist64[i],
            ...
            chi_ring_pos[i] = ring_pos_new
            chi_valid_len[i] = valid_len_new
        ...
        omega12[i] = int(omega_trace4[i, 3])
        has_closed_word[i] = 1
        omega_sig[i] = sig
        parity_O12[i] = O12 & 0x0FFF
        parity_E12[i] = E12 & 0x0FFF
        parity_bit[i] = 0
...
def ingest_word4_batch(
    ...
    has_closed_word_a = _as_writeable_c_array(has_closed_word, dtype=np.uint8, ndim=1, name="has_closed_word")
    ...
    chi_valid_len_a = _as_writeable_c_array(chi_valid_len, dtype=np.uint8, ndim=1, name="chi_valid_len")
    ...
    lib = _get_cpu_lib()
    if lib is None or not hasattr(lib, "gyrograph_ingest_word4_batch"):
        omega_trace4, chi_trace4 = _py_trace_word4_batch(omega12_a, words4_a)
        _py_apply_trace_word4_batch(
            omega12_a,
            step_a,
            last_byte_a,
            has_closed_word_a,
    ...
    lib.gyrograph_ingest_word4_batch(
        _void_p(omega12_a),
        _void_p(step_a),
        _void_p(last_byte_a),
...
def trace_word4_batch(omega12, words4) -> tuple[np.ndarray, np.ndarray]:
    omega12_a = _as_read_c_array(omega12, dtype=np.int32, ndim=1, name="omega12")
    words4_a = _as_read_c_array(words4, dtype=np.uint8, ndim=2, name="words4")

    if words4_a.shape != (omega12_a.shape[0], 4):
        raise ValueError(
            f"words4 must have shape {(omega12_a.shape[0], 4)}, got {words4_a.shape}"
        )

    n = int(omega12_a.shape[0])
    omega_trace4 = np.empty((n, 4), dtype=np.int32)
    chi_trace4 = np.empty((n, 4), dtype=np.uint8)

    lib = _get_cpu_lib()
    if lib is None or not hasattr(lib, "gyrograph_trace_word4_batch"):
        return _py_trace_word4_batch(omega12_a, words4_a)

    lib.gyrograph_trace_word4_batch(
        _void_p(omega12_a),
        _void_p(words4_a),
        ctypes.c_int64(n),
...
def apply_trace_word4_batch(
    ...
    if omega_trace4_a.shape != (n, 4):
        raise ValueError(f"omega_trace4 must have shape {(n, 4)}, got {omega_trace4_a.shape}")
    if chi_trace4_a.shape != (n, 4):
        raise ValueError(f"chi_trace4 must have shape {(n, 4)}, got {chi_trace4_a.shape}")

    lib = _get_cpu_lib()
    if lib is None or not hasattr(lib, "gyrograph_apply_trace_word4_batch"):
        _py_apply_trace_word4_batch(
            omega12_a,
            step_a,
            last_byte_a,
    ...
    lib.gyrograph_apply_trace_word4_batch(
        _void_p(omega12_a),
        _void_p(step_a),
        _void_p(last_byte_a),
        _void_p(has_closed_word_a),
...
def trace_word4_batch_opencl(omega12, words4) -> tuple[np.ndarray, np.ndarray]:
    omega12_a = _as_read_c_array(omega12, dtype=np.int32, ndim=1, name="omega12")
    words4_a = _as_read_c_array(words4, dtype=np.uint8, ndim=2, name="words4")
    if words4_a.shape != (omega12_a.shape[0], 4):
        raise ValueError(
    ...
    if lib is None or not hasattr(lib, "gyrograph_cl_trace_word4_batch"):
        return trace_word4_batch(omega12_a, words4_a)
    ...
    if ok != 1:
        raise RuntimeError("gyrograph_cl_trace_word4_batch failed")
