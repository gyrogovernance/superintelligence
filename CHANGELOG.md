...
- **Synthesis domain** (artificial gene synthesis and design): frozen-model evaluation on *E. coli*, yeast, SARS-CoV-2, and human chromosome 22 splice-flank catalogs. Exact K4 equivariance holds to `3.32e-11`. Trained Super retains order memory on permutation pairs at trained-minus-random margins of `+0.1931` to `+0.2271`. Against GC-matched shuffles, Super climate reaches AUC `0.8408` in *E. coli* and `0.7113` in yeast.
- **Topology domain** (biological membrane topology): within-gene comparison of transmembrane and cytoplasmic coding windows across 590 *E. coli* integral membrane proteins. Mean shell sits `-0.2262` units lower in transmembrane windows (negative in 492 of 590 genes; `p = 0.0002`), and holds under protein-fixed and usage-matched nulls. A frozen Narrow read adds `+0.009094` held-out AUC over a controlled baseline.
- **Fixed-peptide synonymous order** (Topology domain, Gamble 2016, yeast): within 28,504 variants across fixed peptides, Super climate ranks expression at mean Spearman `+0.0196` (`p = 0.0038`), holding across peptide-held-out folds.
...
This release presents the hQVM Group-Equivariant Autoencoder (`src/tools/autoencoder/`), a PyTorch package in which the kernel remains the single authority and every dataset and label is routed through it. Symmetry is exact by construction, not learned: `k4` (Klein gates via Reynolds symmetrization), `AffineSpectralCodec` (full 8192-element affine group over the Walsh spectrum, exact even before training), and `Super` (exact ledger grammar with learned provenance and gated QuBEC climate).

### Shipped

- The verified dictionary: embedding corpora over states, bytes, signatures, and words with exact kernel labels and a one-pass audit (reconstruction, equivariance, shadow invariance, frame parity, psi_hat character-energy identity).
- Over 100 pytest tests covering models, losses, metrics, datasets, reports, and kernel mapping.
- Production models, including Super as a hybrid: frozen exact grammar, learned provenance, gated analytic λ climate (90.0% of the QuBEC ceiling at λ=4), and a sequential residual with an opt-in previous-micro Markov filter (+2.89 bits; gap +0.30 to the `6·h₂(0.1)+2` bound on this holdout). Shipped winner is context 32 / atom 8 (`claim`: `exact_grammar_plus_provenance_and_residual`; sha256 `42e8e5ba40c304e96d0b5d243dab712a0409c3bc4804d8bc86cb10185981c4ff`), regenerated end-to-end by `training_super gates`.
- Super gate regenerability: `evaluate_gates(lite=True)` always runs λ / Markov / boundary measure gates (capacity sweep ranks on them); `main_gates` continue-trains the sweep winner (6 `super_all` + 2 Markov-path polish) to match the documented production recipe.
- The CGM Null Dataset (`dataset_null`): definition, construction from archetype `0xAA` and rest, and layer build order documented in the AE README (with pointers in the AE report and `datasets.py`).
- Bayes-optimal spectral denoising on `AffineSpectralCodec`, with closed-form gains available on the Walsh occupation codec.
- A single production regeneration path via `helpers.training_super production` (narrow/general nulls, Super gates, `production_summary.json`).

### CGM Science in parallel

The geometry this package instantiates came from the [science](https://github.com/gyrogovernance/science) program, which develops the Common Governance Model, the mathematical framework behind the kernel. Here is the recent record in plain terms:

- **[1.3.9.7-CGM_Genomics]** (08-25 to 08-27). We showed that the genome follows the same structural ladder as the hQVM kernel. A nucleotide maps to a two-bit value, a codon to one of sixty-four states, an overlapping four-letter window to a single byte instruction, and an ordered codon pair to a point of the 4096-state carrier. In this picture the genetic code is a quotient of codon space in which synonymous codons share a meaning while keeping their own identity, and the stop and serine codons act as the two generators of the semantic fold plane. A 301-check verification suite ran green, and among the NCBI genetic code tables only tables 1 and 11 satisfy all constraints. [Analysis](https://github.com/gyrogovernance/science/blob/main/docs/Findings/Analysis_hQVM_CGM_Genomics.md)

- **[1.3.9.6-CGM]** (08-24). We sharpened the framework's account of dual balance and path memory, and we adopted a cleaner naming discipline. The word holonomy is now reserved for the path-memory operator, while the quantity formerly called by that name is the BU dual-pole loop angle. [Analysis](https://github.com/gyrogovernance/science/blob/main/docs/Findings/Analysis_Holonomy.md)

- **[1.3.9.5-CGM_hQVM_Group_Theory]** (08-22 to 08-23). We completed the description of the finite symmetry group generated by the kernel's byte alphabet. The 256 byte instructions generate a group of order 8192 acting on the 4096 states, and its representation structure is now fully characterized with 2080 irreducible sectors. The spectral autoencoder instantiates exactly this structure, block for block, so the group theory and the model are the same object described twice. [Analysis](https://github.com/gyrogovernance/science/blob/main/docs/Findings/Analysis_hQVM_CGM_Group_Theory.md)

- **[1.3.9.4-CGM_Precession]** (08-17 to 08-18). We analyzed precession in the kernel, the same effect by which a gyroscope's axis drifts as it is carried around a closed path. The work covers three transport prescriptions, orthogonal pairs of Thomas-Wigner precessions, and a spectrum of closed walks of lengths two through five. [Analysis](https://github.com/gyrogovernance/science/blob/main/docs/Findings/Analysis_Precession.md)

- **[1.3.9.3-CGM]** (08-15 to 08-16). We re-derived the aperture, one of the framework's foundational constants, from first principles. The BU dual-pole loop angle is now given by the closed form δ_BU = 4·arctan(k(π/4)·k(m_a)) instead of a fitted number. The numeric change is tiny, about eight parts per billion, but the equation now carries the derivation, and the fine-structure constant computed from it sits 33.8 ppb from the CODATA 2018 value. [Analysis](https://github.com/gyrogovernance/science/blob/main/docs/Findings/Analysis_Holonomy.md)

---

## [v2.1.0-Gyroscopic] – 2026-04-19 to 2026-08-11
...
The graph ends closed next. Final normalization and the output projection run on the native driver. Deterministic greedy emission selects the token from the same controller contraction, so the host sampler chain is not the decision law. Request entry into Genealogy uses a named Common Source projection from the embedding signs, while GGUF may still supply the residual seed as chassis storage.
...
We own the 36 block forward pass, the final norm, the output projection, and deterministic greedy emission. Genealogy runs as one continuous history across prompt and generation, spanning tokens times layers exactly. Attention memory, rotary phase, normalization, the feed forward gate, the residual stream, and the named entry projection all follow native law. 

What remains is algebraic closure of the remaining floating interoperability charts: residual and scratch row buffers, attention accumulation, and row writeback. Activation scales are already stored as finite half-precision bits, and aperture gains use a fixed Q16 chart. Quality and speed acceptance follow once those buffers are gone.
