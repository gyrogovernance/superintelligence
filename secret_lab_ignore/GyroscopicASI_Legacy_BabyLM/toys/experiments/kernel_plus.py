...
def tensor_to_int(tensor: np.ndarray[Any, Any]) -> int:  # type: ignore
...
def int_to_tensor(state_int: int) -> np.ndarray[Any, Any]:  # type: ignore
                ...
                with safe_open(st_path, framework="pt", device="cpu") as f:
                    for key in f.keys():
                        ...
                        if isinstance(tensor, torch.Tensor):
                            try:
                                arr = tensor.detach().cpu().numpy()  # type: ignore
                            except TypeError:
                                arr = tensor.detach().cpu().float().numpy()  # type: ignore
...
class GyroKernel:
    ...
    def _import_model_weights(self, model_data: dict[str, Any]) -> None:  # type: ignore
        ...
        if not self.virtual_tokens and weights:
            ...
            try:
                ...
                np.savez_compressed(str(vt_cache), virtual_tokens=self.virtual_tokens)  # type: ignore
                if self.debug:
    ...
    def _project_embeddings_to_token_states(self, embeddings: np.ndarray[Any, Any]) -> None:  # type: ignore
    ...
    def _resonance_defect(self, token_id: int) -> int:
        ...
        if hasattr(self, "weight_lens") and self.weight_lens is not None:  # type: ignore
            try:
                exon = self.weight_lens.exon_for_token(int(token_id))  # type: ignore
