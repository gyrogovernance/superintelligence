...
class RubricItem:
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]):  # type: ignore
        return cls(
...
def calculate_score(rubric_items: list[RubricItem], grading_response_list: list[dict[str, Any]]) -> float | None:  # type: ignore
...
class HealthBenchEval(Eval):
    ...
    def grade_sample(
    ...
    ) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:  # type: ignore
        # construct and grade the sample
        convo_with_response = prompt + [dict(content=response_text, role="assistant")]

        def grade_rubric_item(rubric_item: RubricItem) -> dict[str, Any]:  # type: ignore
        ...
        for tag, items_grades in rubric_tag_items_grades.items():
            ...
            score = calculate_score(list(items), list(grades))  # type: ignore
    ...
    def __call__(self, sampler: SamplerBase) -> EvalResult:
        def fn(row: dict[str, Any]):  # type: ignore
