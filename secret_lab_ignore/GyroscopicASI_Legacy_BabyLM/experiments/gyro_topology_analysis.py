...
class TopologyReporter:
    ...
    def generate_sector_report(
        ...
        if fermion_data.clicks_e21 < 0.2:
            report.append("  * Near-orthogonal sector (clean 45 deg backbone)")
            report.append("  * Minimal aperture perturbations")
            report.append("  * Expected: Low plane flips, high closure rate")
        elif fermion_data.clicks_e21 > 2.0:
            report.append(f"  * High-aperture sector (~{fermion_data.clicks_e21:.1f} clicks)")
            report.append("  * Strong 3 deg quantization effects")
            report.append("  * Expected: More plane flips, complex oscillations")
        else:
            report.append(f"  * Moderate aperture sector (~{fermion_data.clicks_e21:.1f} clicks)")
            report.append("  * Intermediate dynamics")

        if metrics['closure_720_rate'] > 0.8:
            report.append("  * Strong 720 deg closure -> stable particle-like behavior")
        elif metrics['closure_720_rate'] > 0.5:
            report.append("  * Near closure -> oscillatory behavior")
        else:
            report.append("  * Weak closure -> chaotic/dissipative dynamics")

        if metrics['avg_30fold_alignment'] > 0.8:
            report.append("  * Excellent 30-fold alignment -> clean phase structure")
        elif metrics['avg_30fold_alignment'] > 0.5:
            report.append("  * Good 30-fold alignment -> structured with noise")
        else:
            report.append("  * Poor 30-fold alignment -> disordered phase")
    ...
    def generate_comparative_report(self, all_results: dict[str, Any]) -> str:
        ...
        report.append(f"\n{'='*8}")
        report.append("COMPARATIVE TOPOLOGY ANALYSIS")
        report.append(f"{'='*8}")
        ...
        if all(rate == 0.0 for rate in closure_720_rates):
            report.append("  * Highest 720 deg closure: none (all sectors at 0.0%)")
        else:
        ...
        report.append("  * Aperture click hierarchy:")
        ...
        report.append("  * CGM stage progression patterns:")
        for sector in sectors:
        ...
        if not np.isnan(cross_corr['correlation']):
            ...
            report.append("\nCross-Sector Correlation:")
            report.append("  Correlation computation failed or insufficient data")
